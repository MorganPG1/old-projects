...
class DTC:
    def __init__(self, dtc, part, type, sub_sys, desc) -> None:
        self.dtc = dtc
        self.part = part
        self.type = type
    ...
    def __repr__(self) -> str:
        return f"{self.dtc} - {self.description}"
...
class DTCParser:
    ...
    def parse_code_asDTC(self, code:str) -> DTC:
        """
        Parses the provided DTC and returns the results as a DTC object

        :param code: DTC to be parsed
        """
        assert len(code) == 5
        part = self.parse_vehicle_part(code[0])
        type = self.parse_code_type(code[1])
        subsys = self.parse_vehicle_subsystem(code[0], code[2])
        desc = self.parse_fault_description(code[0] + code[1], code[2] + code[3] + code[4])

        return DTC(code, part, type, subsys, desc)
